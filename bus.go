...
func busAddDevice(devNum int, mnem string, pmb int, att bool, io bool, boot bool) {
	...
	debugPrint(DEBUG_LOG, "INFO: Device 0%o added to bus\n", devNum)
}

func busSetDataInFunc(devNum int, fn DataInFunc) {
	d[devNum].dataInFunc = fn
	debugPrint(DEBUG_LOG, "INFO: Bus Data In function set for dev #0%o\n", devNum)
}
...
func busSetDataOutFunc(devNum int, fn DataOutFunc) {
	...
	debugPrint(DEBUG_LOG, "INFO: Bus Data Out function set for dev #0%o\n", devNum)
}
...
func busDataOut(cpuPtr *Cpu, iPtr *DecodedInstr, abc byte) {
	...
	debugPrint(DEBUG_LOG, "INFO: Bus Data Out function called for dev #0%o\n", iPtr.ioDev)
}

func busSetResetFunc(devNum int, resetFn ResetFunc) {
	d[devNum].resetFunc = resetFn
	debugPrint(DEBUG_LOG, "INFO: Bus reset function set for dev #0%o\n", devNum)
}
...
func busSetBusy(devNum int, f bool) {
	d[devNum].busy = f
	debugPrint(DEBUG_LOG, "... Busy flag set to %d for device #0%o\n", boolToInt(f), devNum)
}

func busSetDone(devNum int, f bool) {
	d[devNum].done = f
	debugPrint(DEBUG_LOG, "... Done flag set to %d for device #0%o\n", boolToInt(f), devNum)
}
